...
_tt_launch() {
  ...
  if [[ $host == 'WSL' ]]; then
    # Local — bash directly (no transport). tt-launch is bootstrap-deployed
    # at ~/.local/bin/tt-launch (a symlink to the repo file).
    local remote="TT_HOST_ALIAS=WSL exec tt-launch '$session'"
    if command -v wt.exe >/dev/null 2>&1; then
      wt.exe -w 0 new-tab --title "WSL:$session" wsl.exe -- bash -lc "$remote"
    else
      eval "$remote"
    fi
  else
    ...
    # reverse-tunnel, and invokes et with a short remote command that calls
    # the bootstrap-deployed tt-launch.
    if command -v wt.exe >/dev/null 2>&1; then
      wt.exe -w 0 new-tab --title "$host:$session" \
        wsl.exe -- bash -lc "tt-et-launch '$host' '$session'"
    else
